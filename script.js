const projectsData = [
        ...
        contactForm.addEventListener('submit', (e) => {
            ...
            let hasError = false;
            if (formData.name.length < 2) { showError('name', 'Name must be at least 2 characters'); hasError = true; }
            if (!validateEmail(formData.email)) { showError('email', 'Please enter a valid email address'); hasError = true; }
            if (formData.message.length < 10) { showError('message', 'Message must be at least 10 characters'); hasError = true; }
